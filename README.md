...
- El logo es el archivo real `assets/logo.png` (el isologo oficial de
  Tarjeta Sucrédito), usado en el header y en el footer. Si en algún
  momento lo reemplazan por una versión nueva, basta con sobrescribir
  ese mismo archivo — no hay que tocar el HTML.
- Arriba hay un **banner que rota solo** (carrusel) con las imágenes
  de los shows a tamaño controlado; tocar una imagen del banner
  también lleva directo a comprar esa entrada.
- Debajo, la sección "Elegí tu show" muestra la lista completa en
  formato compacto (miniatura chica + datos + botón), para elegir
  y comprar sin que ninguna imagen ocupe media pantalla.
- Banner y lista se arman automáticamente desde el mismo archivo
  `js/events.js` — un solo lugar para cargar cada show, se refleja
